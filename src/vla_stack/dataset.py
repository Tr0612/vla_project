...
def pil_to_chw_tensor(image: Image.Image) -> torch.Tensor:
    # Force a writable backing buffer to avoid torch warning on read-only NumPy views from PIL.
...
def sample_get_action_chunk(sample: dict[str, Any]) -> torch.Tensor:
    if "action_chunk" in sample:
        chunk = torch.as_tensor(sample["action_chunk"], dtype=torch.float32)
        if chunk.ndim == 1:
            return chunk.unsqueeze(0)
        return chunk
    return sample_get_action(sample).unsqueeze(0)
...
class VLAJsonlDataset(UnifiedSampleAdapter):
    ...
    def __init__(
        ...
        self.action_chunk_size = int(max(1, action_chunk_size))
        ...
        self._action_by_episode_step: dict[tuple[str, int], torch.Tensor] = {}
        self._max_step_by_episode: dict[str, int] = {}
        for idx, s in enumerate(self.samples):
            ep = s.episode_id if s.episode_id else f"jsonl_{idx}"
            ts = int(s.timestep)
            a = torch.tensor(s.action, dtype=torch.float32).reshape(-1)
            self._action_by_episode_step[(ep, ts)] = a
            self._max_step_by_episode[ep] = max(self._max_step_by_episode.get(ep, ts), ts)
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        max_step = int(self._max_step_by_episode.get(episode_id, timestep))
        for off in range(self.action_chunk_size):
            step = min(timestep + off, max_step)
            a = self._action_by_episode_step.get((episode_id, step))
            if a is None:
                a = action
            action_chunk.append(a)
        action_chunk_tensor = torch.stack(action_chunk, dim=0)
...
class ShortMetaWorldDataset(UnifiedSampleAdapter):
    ...
    def __init__(
        ...
        self._action_by_key: dict[tuple[str, int, int], torch.Tensor] = {}
        self._max_step_by_traj: dict[tuple[str, int], int] = {}
        for row in self.samples:
            ...
            self._action_by_key[key] = torch.as_tensor(row["action"], dtype=torch.float32).reshape(-1)
            traj_key = (str(row["task_name"]), int(row["trajectory_id"]))
            self._max_step_by_traj[traj_key] = max(self._max_step_by_traj.get(traj_key, int(row["step_id"])), int(row["step_id"]))
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        action_chunk: list[torch.Tensor] = []
        max_step = int(self._max_step_by_traj.get((task_name, trajectory_id), step_id))
        for off in range(self.action_chunk_size):
            step = min(step_id + off, max_step)
            a = self._action_by_key.get((task_name, trajectory_id, step))
            if a is None:
                a = action
            action_chunk.append(a)
        action_chunk_tensor = torch.stack(action_chunk, dim=0)
